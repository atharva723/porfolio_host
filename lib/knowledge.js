...
// Projects are kept in a decoupled JSON file (updated by the automation
// workflow). Read it so the chat knowledge base stays in sync with the cards
// shown on the site; fall back to the built-in list in data.js if it's missing
// or invalid.
function getProjectItems() {
  try {
    ...
    /* fall back to data.js projects below */
  }
  return (portfolioData && portfolioData.projects && portfolioData.projects.items) || [];
}

function getPortfolioData() {
  return {
    ...portfolioData,
    projects: { ...(portfolioData.projects || {}), items: getProjectItems() },
  };
}
...
function buildSystemPrompt() {
  // In development, skip the cache so edits to knowledge.md take effect on the
  // next request without a restart (matches this file's documented workflow).
  // In production the content is static, so cache it for performance.
  if (cached && process.env.NODE_ENV === "production") return cached;

  const name = portfolioData?.site?.name || "the portfolio owner";
  const knowledge = portfolioToText(getPortfolioData());
